...
var config = DefaultConfig.Instance
    ...
    .HideColumns(BenchmarkDotNet.Columns.Column.BuildConfiguration)
    ;

config.AddDiagnoser(MemoryDiagnoser.Default);

var localJob = Job.ShortRun
    .WithToolchain(CsProjCoreToolchain.NetCoreApp10_0) // .NET 10
    .DontEnforcePowerPlan()
    .WithId("v2");

// job for NuGet version(v1)
...
config.AddJob(localJob);
config.AddJob(nugetVersionJob);
